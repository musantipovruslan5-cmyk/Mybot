...
import telebot
from config import TOKEN, admin, chat_bota, instruction, nicknameadm, procent, token_qiwi, admin2, instruction, chat_bota, chat_id_bot, channel_id
...
def start(message: types.Message):
	...
	some_var = bot.get_chat_member(channel_id, chat_id)
	...
	if message.from_user.username == None:
		...
		if user_status == 'member' or user_status == 'administrator' or user_status =='creator':
			...
			bot.send_message(chat_id, '🖲 Чтобы пользоваться гарант-ботом, вы должны вступить в Инфо-канал Crips, иначе доступ запрещен. После подписки напишите /start', 
				reply_markup = kb.channel_sub(), 
			...
			if info[0] == '1':
				...
				if info != None:
					...
					if message.text.lower() == '👤 профиль':
						...
						bot.send_message(chat_id, 'Выберите игру, или создайте свою:', reply_markup = dice.dice_menu())
		else:
			bot.send_message(message.from_user.id, '🖲 Чтобы пользоваться гарант-ботом, вы должны вступить в Инфо-канал Crips, иначе доступ запрещен. После подписки напишите /start', 
					reply_markup = kb.channel_sub())
...
def handler_call(call):
	...
	if chat_id.startswith('-'):
		...
		if call.data == 'my_games:dice':
			info = dice.my_games_dice(chat_id)
			bot.send_message(chat_id, 
							text=info, reply_markup = dice.my_games_cancel(chat_id))
		
		if 'games_user:' in call.data:
			game_id = call.data.split(':')[1]
			info = dice.get_info_games(game_id)
			bot.send_message(chat_id,
					text=info[0],
					reply_markup=info[1])
		...
		if 'game_del:' in call.data:
			...
			bot.delete_message(chat_id, message_id)
			bot.send_message(chat_id, f'Игра успешно удалена, на ваш баланс вернулись все средства!')

		if call.data == 'create_dice':
			msg = bot.send_message(chat_id, '💰 Введите сумму ставки от 10 RUB')
			bot.register_next_step_handler(msg, create_dice_games)
		...
		if 'dice_game:' in call.data:
			...
			if game.status == True and game.user_id != str(chat_id):
				...
				if info == False:
					...
					bot.send_message(chat_id, text=info[0], reply_markup=info[1])
			else:
				bot.send_message(chat_id, 'Нельзя играть с самим собой!')

		if 'start_game_dice:' in call.data:
			game = dice.Game(call.data.split(':')[1])
			balance = func.profile(chat_id)[2]
			if game.status == True and game.user_id != str(chat_id):
				if float(balance) >= float(game.bet):
					dice.main_start(game, bot, chat_id)
				else:
					bot.send_message(chat_id, '❌Для игры пополните баланс')
			else:
		...
		if user_status == 'member' or user_status == 'administrator' or user_status =='creator':
			if call.data == 'output':
				...
				if check == None:
					...
					bot.send_message(admin,text=f'✅ Произошло пополнение баланса QIWI!\n\nСумма {check}\n\nПользователь @{func.profile(chat_id)[5]}')
					bot.send_message(admin2, f'✅ Произошло пополнение баланса QIWI!\n\nСумма {check}\n\nПользователь @{func.profile(chat_id)[5]}')
			...
			bot.send_message(chat_id, 
				text = '🖲 Чтобы пользоваться гарант-ботом, вы должны вступить в наш Инфо-канал Crips, иначе доступ запрещен. После подписки напишите /start', 
				reply_markup = kb.channel_sub())
		...
		if message.text.startswith('-'):
			...
			if float(money) > float(balance):
				...
				if float(money) < 10:
					...
					bot.send_message(admin, text='✅ Отправлен запрос на вывод!\nQiwi - <b><code>{qiwi}</code></b>\nСумма - <b><code>{money}</code></b> рублей'.format(qiwi=info[3], money=result), 
						parse_mode='HTML')
					bot.send_message(admin2, text='✅ Отправлен запрос на вывод!\nQiwi - <b><code>{qiwi}</code></b>\nСумма - <b><code>{money}</code></b> рублей'.format(qiwi=info[3], money=result), 
						parse_mode='HTML')
	except:
		bot.send_message(message.chat.id,text=error)
...
def create_dice_games(message):
        bet = message.text
        try:
            balance = func.profile(message.from_user.id)[2]
            if int(bet) >= 10:
                if float(balance) >= float(bet):
                    id_games = random.randint(111, 999)
                    func.update_balances(message.from_user.id, bet)
                    dice.create_game(id_games, message.from_user.id, bet)
                    bot.send_message(chat_id=message.from_user.id, text='Ставка принята')
                else:
                    bot.send_message(message.from_user.id, 'Пополните баланс!')
            else:
                bot.send_message(message.from_user.id, f'Минимальная ставка 10 RUB') #
        except Exception as e:
        	print(e)
            #bot.send_message(message.from_user.id, 'Ошибка, сообщите поддержке')
                ...
                for i in payouts:
                    if i[1] == '0':
                        bot.send_message(chat_id=i[0],
                                        text=f'Вы отправили пустой чек :(')
                        cursor.execute(f'DELETE FROM payouts WHERE user_id = "{i[0]}"')
                        conn.commit()
                    else:
                        func.update_balance(i[0], i[1])
                        ...
                        bot.send_message(
                                chat_id=admin,
                                text=f'Пришло пополнение Banker!\n\n'
                                     f'От: @{func.profile(i[0])[1]}\n\n'
                                     f'Сумма: {i[1]} RUB')
                        bot.send_message(
                                chat_id=admin2,
                                text=f'Пришло пополнение Banker!\n\n'
...
while True:
	try:
		...
		bot.send_message(1093342102, f'Появилась ошибка: \n\n{e}')
		sleep(1)


try:
	threading.Thread(target=check_payouts).start()
